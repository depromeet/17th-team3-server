...
/**
 * Place 검색 부하 테스트 - Coroutine (병렬-Non-blocking) 모드
 * 
 * 기본값: processingMode=coroutine
 * 
 * 사용 방법:
 *  - nGrinder에서 이 스크립트를 선택하고 테스트 실행
 *  - 스크립트 리소스 입력 불필요 (기본값으로 coroutine 사용)
 */
@RunWith(GrinderRunner)
class PlaceMockLoadCoroutineScenario {

    private static final GTest TEST_PLACE_SEARCH = new GTest(1, "Place 검색 (Coroutine)")
    ...
    static void beforeProcess() {
        HTTPPluginControl.getConnectionDefaults().setTimeout(60_000)  // 60초 타임아웃
        
        baseUrl = System.getProperty("baseUrl", "http://host.docker.internal:8080").trim()
        processingMode = System.getProperty("processingMode", "coroutine").trim().toLowerCase()  // 기본값: coroutine
        ...
        grinder.logger.info("================================================")
        grinder.logger.info("===== Place 검색 부하 테스트 (COROUTINE) =====")
        grinder.logger.info("================================================")
    ...
    void execute() {
        ...
        // 응답 검증 (Coroutine은 가장 빠를 것으로 예상)
        def slowThreshold = 3000
        
        if (response.statusCode != 200) {
            grinder.logger.warn("[COROUTINE] 요청 실패 - Status: ${response.statusCode}, meetingId: ${meetingId}, userId: ${userId}, 응답시간: ${elapsedTime}ms")
        } else if (elapsedTime > slowThreshold) {
            grinder.logger.warn("[COROUTINE] 느린 응답 - meetingId: ${meetingId}, userId: ${userId}, 응답시간: ${elapsedTime}ms")
        } else {
            grinder.logger.debug("[COROUTINE] 성공 - meetingId: ${meetingId}, userId: ${userId}, 응답시간: ${elapsedTime}ms")
        }
        ...
        if (totalRequests % 100 == 0) {
            synchronized(statsLock) {
                ...
                grinder.logger.info("===============================================")
                grinder.logger.info("===== [COROUTINE] 진행 상황 (${totalRequests}건) =====")
