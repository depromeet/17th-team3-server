...
/**
 * Place 검색 부하 테스트 - ThreadPool (병렬-Blocking) 모드
 * 
 * 기본값: processingMode=threadpool
 * 
 * 사용 방법:
 *  - nGrinder에서 이 스크립트를 선택하고 테스트 실행
 *  - 스크립트 리소스 입력 불필요 (기본값으로 threadpool 사용)
 */
@RunWith(GrinderRunner)
class PlaceMockLoadThreadPoolScenario {

    private static final GTest TEST_PLACE_SEARCH = new GTest(1, "Place 검색 (ThreadPool)")
    ...
    static void beforeProcess() {
        ...
        processingMode = System.getProperty("processingMode", "threadpool").trim().toLowerCase()  // 기본값: threadpool
        ...
        grinder.logger.info("================================================")
        grinder.logger.info("===== Place 검색 부하 테스트 (THREADPOOL) =====")
        grinder.logger.info("================================================")
    ...
    void execute() {
        ...
        // 응답 검증
        def slowThreshold = 5000
        
        if (response.statusCode != 200) {
            grinder.logger.warn("[THREADPOOL] 요청 실패 - Status: ${response.statusCode}, meetingId: ${meetingId}, userId: ${userId}, 응답시간: ${elapsedTime}ms")
        } else if (elapsedTime > slowThreshold) {
            grinder.logger.warn("[THREADPOOL] 느린 응답 - meetingId: ${meetingId}, userId: ${userId}, 응답시간: ${elapsedTime}ms")
        } else {
            grinder.logger.debug("[THREADPOOL] 성공 - meetingId: ${meetingId}, userId: ${userId}, 응답시간: ${elapsedTime}ms")
        }
        ...
        if (totalRequests % 100 == 0) {
            synchronized(statsLock) {
                ...
                grinder.logger.info("===============================================")
                grinder.logger.info("===== [THREADPOOL] 진행 상황 (${totalRequests}건) =====")
